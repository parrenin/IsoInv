...
# Model function
    ...
    def model1D(self, j):
        ...
        #FIXME: take into account temporal variations of ice thickness
        self.Tf[j] = Tf(rhog*ggrav*self.thkie[j])
        #We assume first that we have melting point everywhere
        self.Tm[j] = (self.Ts+self.Tf[j])/2
        self.alpha[j] = m.sqrt(self.a[j]/365./24./3600./self.thk[j]/Kg(self.Tm[j], 1.)*rhog*\
                        cg(self.Tm[j])/2.)
        self.DeltaT[j] = self.Ts-self.Tf[j]
        self.G[j] = -self.DeltaT[j]*2*Kg(self.Tm[j], 1.)*self.alpha[j]/m.sqrt(m.pi)/\
                    erf(self.thkie[j]*self.alpha[j])
        self.is_fusion[j] = (self.G0[j] > self.G[j])
        self.G[j] = min(self.G[j], self.G0[j])
        self.m[j] = (self.G0[j]-self.G[j])*365.242*24*3600/rhog/Lf
        self.T[:, j] = self.Ts-self.G[j]*m.sqrt(m.pi)/2./Kg(self.Tm[j], 1.)/self.alpha[j]*\
                       (erf(self.alpha[j]*self.zeta[:, j]*self.thkie[j])-\
                       erf(self.alpha[j]*self.thkie[j]))
        self.T_anal[:, j] = self.T[:, j]+0.

        #Mechanical model
        self.mu[j] = self.m[j]/self.a[j]
        #Parrenin et al. (CP, 2007a) 2.2 (3)
        self.omega_D[:, j] = 1-(self.p[j]+2)/(self.p[j]+1)*(1-self.zetaie[:, j])+1/(self.p[j]+1)*\
                             (1-self.zetaie[:, j])**(self.p[j]+2)
        ...
        for it in range(self.tm_iter):
            ...
            if self.is_fusion[j]:
                ...
                self.Atemp = 1/(self.dzeta*self.thk[j])*(np.diag(Kg((self.T[1:-2, j]+\
                             self.T[2:-1, j])/2, self.D[1:-1, j]), 1)+np.diag(Kg((self.T[1:-2, j]\
                             +self.T[2:-1, j])/2, self.D[1:-1, j]), -1)\
                             +np.diag(-Kg((self.T[:-2, j]+self.T[1:-1, j])/2, self.D[:-1, j])\
                             -Kg((self.T[1:-1, j]+self.T[2:, j])/2, self.D[1:, j]), 0))
                self.Atemp = self.Atemp-0.5*rhog*np.transpose([(self.D[:-1, j]+self.D[1:, j])/\
                             2*self.uz[1:-1, j]*cg(self.T[1:-1, j])])*(np.diag(np.ones(np.size\
                             (self.zetagrid)-3), -1)-np.diag(np.ones(np.size(self.zetagrid)-3), 1))   #TODO:We are missing a term due to heat production by deformatio
                self.Btemp[0] = -(Kg((self.T[0, j]+self.T[1, j])/2, self.D[0, j])/self.dzeta/\
                                self.thk[j]-0.5*rhog*(self.D[0, j]+self.D[1, j])/2*\
                                cg(self.T[1, j])*self.uz[1, j])*self.Ts
                self.Btemp[-1] = -(Kg((self.T[-2, j]+self.T[-1, j])/2, self.D[-1, j])/self.dzeta/\
                                 self.thk[j]+0.5*rhog*(self.D[-1, j]+self.D[-2, j])/2*\
                                 cg(self.T[-2, j])*self.uz[-2, j])*self.Tf[j]
                self.T[1:-1, j] = np.linalg.solve(self.Atemp, self.Btemp)
                self.G[j] = -Kg((self.T[-1, j]+self.T[-2, j])/2, self.D[-1, j])*(self.T[-2, j]-\
                            self.T[-1, j])/(self.depthie[-1, j]-self.depthie[-2, j])
                self.m[j] = (self.G0[j]-self.G[j])*365.242*24*3600/rhog/Lf
                ...
                self.Atemp = 1/(self.dzeta*self.thk[j])*(np.diag(Kg((self.T[1:-2, j]+\
                             self.T[2:-1, j])/2, self.D[1:-1, j]), 1)+\
                             np.diag(Kg((self.T[1:-2, j]+self.T[2:-1, j])/2, self.D[1:-1, j])\
                             , -1)+np.diag(-Kg((self.T[:-2, j]+self.T[1:-1, j])/2,\
                             self.D[:-1, j])-Kg((self.T[1:-1, j]+self.T[2:, j])/2,\
                             self.D[1:, j]), 0))
                #TODO:We are missing a term due to heat production by deformation
                self.Atemp = self.Atemp-0.5*rhog*np.transpose([(self.D[:-1, j]+self.D[1:, j])/\
                             2*self.uz[1:-1, j]*cg(self.T[1:-1, j])])*(np.diag(np.ones(np.size(\
                             self.zetagrid)-3), -1)-np.diag(np.ones(np.size(self.zetagrid)-3), 1))
                vector = np.concatenate((np.zeros(np.size(self.zetagrid)-3), np.array([ Kg((\
                         self.T[-2, j]+self.T[-1, j])/2, self.D[-1, j])/self.dzeta/self.thk[j]\
                         - 0.5*rhog*(self.D[-1, j]+self.D[-2, j])/2*cg(self.T[-2, j])*\
                         self.uz[-2, j] ]) ))
                self.Atemp = np.concatenate(( self.Atemp, np.array([ vector ]).T ), axis=1)
                vector = np.concatenate(( np.zeros(np.size(self.zetagrid)-3), Kg((self.T[-2, j]\
                         +self.T[-1, j])/2, self.D[-1, j])/self.dzeta/self.thk[j] *\
                         np.array([1, -1]) ))
                self.Atemp = np.concatenate(( self.Atemp, np.array([ vector ])), axis=0)
                #Is there an indice problem here?
                self.Btemp[0] = -(Kg((self.T[0, j]+self.T[1, j])/2, self.D[0, j])/self.dzeta/\
                                self.thk[j]-0.5*(self.D[0, j]+self.D[1, j])/2*rhog*\
                                cg(self.T[0, j])*self.uz[1, j])*self.Ts
        ...
        return np.concatenate(( np.array([self.a[j]]), np.array([self.m[j]]),\
               np.array([self.pprime[j]]), self.age[:, j], np.log(self.age[1:, j]-\
...
#        self.m = variables[np.size(self.distance):2*np.size(self.distance)]
        ...
        resi = np.concatenate((resi, np.array([ (self.pprime[j]-self.pprime_prior)/\
               self.pprime_sigma ]) ))
        if self.invert_G0:
...
#       return jacob
    ...
    def sigma(self):
        ...
        index = 0
        c_model = np.dot(np.transpose(jacob[:, index:index+np.size(self.a)]),
                         np.dot(self.hess, jacob[:, index:index+np.size(self.a)]))
        self.sigma_a = np.sqrt(np.diag(c_model))
        index = index+np.size(self.a)
        c_model = np.dot(np.transpose(jacob[:, index:index+np.size(self.m)]),
                         np.dot(self.hess, jacob[:, index:index+np.size(self.m)]))
        self.sigma_m = np.sqrt(np.diag(c_model))
        index = index+np.size(self.m)
        c_model = np.dot(np.transpose(jacob[:, index:index+np.size(self.p)]),
                         np.dot(self.hess, jacob[:, index:index+np.size(self.p)]))
        self.sigma_pprime = np.sqrt(np.diag(c_model))
        index = index+np.size(self.p)
        c_model = np.dot(np.transpose(jacob[:, index:index+np.size(self.age)]),
                         np.dot(self.hess, jacob[:, index:index+np.size(self.age)]))
        self.sigma_age = np.sqrt(np.diag(c_model))
        self.sigma_age = np.reshape(self.sigma_age, (np.size(self.zetagrid),
                                                     np.size(self.distance)))
        index = index+np.size(self.age)
        c_model = np.dot(np.transpose(jacob[:, index:index+np.size(self.G0)]),
                         np.dot(self.hess, jacob[:, index:index+np.size(self.G0)]))
...
#        plt.legend()
        ...
        plotpara = plt.subplot(713,
                               aspect=self.aspect/(m.log(self.p_max+1)-\
                                                   m.log(self.p_min+1))*self.max_depth/7)
...
#            plt.yaxis.set_label_position('right')
    ...
    def bot_age_save(self):
        ...
        with open(self.label+'agebottom.txt', 'w') as f:
            f.write('#LON\tLAT\tdistance(km)\tthickness(m)\tage60m(yr-b1950)\tage-min(yr-b1950)'
...
#        if RL.thk[j]<>np.nan and 
    ...
    for j in range(np.size(RL.distance)):
        ...
        #FIXME: This does not work since the depth grid varies!
...
#            print RL.variables1D
        ...
        RL.reso1dot5Myrmin[j] = np.percentile(np.where(np.isnan(resolution_accepted), 0.,
                          resolution_accepted), 15)
        RL.reso1dot5Myrmax[j] = np.percentile(np.where(np.isnan(resolution_accepted), 0.,
                          resolution_accepted), 85)
        RL.height1dot5Myrmin[j] = np.percentile(np.where(np.isnan(height1dot5Myr_accepted), 0.,
                            height1dot5Myr_accepted), 15)
        RL.height1dot5Myrmax[j] = np.percentile(np.where(np.isnan(height1dot5Myr_accepted), 0.,
                            height1dot5Myr_accepted), 85)
        ...
        RL.sigma_reso1dot5Myr = np.std(np.where(np.isnan(resolution_accepted), 0.,
                                                resolution_accepted))
